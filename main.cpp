template <typename Number = double> constexpr Number Pi{3.1415926535};

double Cos(double x, unsigned long long p = 8) {

    x -= static_cast<unsigned long long>(x/Pi<double>)*Pi<double>;
    ...
    x -= static_cast<unsigned long long>(x/(Pi<double>*2))*(Pi<double>*2);
...
double Sin(double x, unsigned long long p = 8) { return Cos(x-Pi<double>/2, p); }
...
double Csc(double x, unsigned long long p = 8) { return Sec(x-Pi<double>/2, p); }
